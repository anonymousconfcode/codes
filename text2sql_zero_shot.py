...
if __name__ == "__main__":
    ...
    if opt.load_in_4bit:
        model = AutoModelForCausalLM.from_pretrained(opt.llm_path, device_map = "auto", torch_dtype = torch.float16, load_in_4bit = True)
    elif opt.load_in_8bit:
        model = AutoModelForCausalLM.from_pretrained(opt.llm_path, device_map = "auto", torch_dtype = torch.float16, load_in_8bit = True)
    else:
