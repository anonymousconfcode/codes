...
if __name__ == "__main__":
    ...
    if "starcoder" in opt.llm_path: # for starcoder
    ...
    elif opt.llm_path == "Salesforce/codegen2-7B" or opt.llm_path == "Salesforce/codegen2-16B": # Codegen2, but slow inference speed
        tokenizer = AutoTokenizer.from_pretrained(opt.llm_path)
        from transformers import CodeGenForCausalLM
        model = CodeGenForCausalLM.from_pretrained(opt.llm_path, # trust_remote_code = True, revision = "main"
            device_map = "auto").half()
        tokenizer.pad_token_id = tokenizer.eos_token_id
        model.config.pad_token_id, model.config.eos_token_id, model.config.bos_token_id = [tokenizer.eos_token_id] * 3
    elif opt.llm_path == "Salesforce/codegen25-7b-multi" or opt.llm_path == "Salesforce/codegen25-7b-mono": # Codegen2.5
        tokenizer = AutoTokenizer.from_pretrained(opt.llm_path, trust_remote_code = True)
        model = AutoModelForCausalLM.from_pretrained(opt.llm_path, device_map = "auto").half()
        tokenizer.pad_token_id = model.config.pad_token_id
        tokenizer.bos_token_id = model.config.bos_token_id
    # elif opt.llm_path == "THUDM/codegeex2-6b": # for codegeex-2
    #     tokenizer = AutoTokenizer.from_pretrained(opt.llm_path, trust_remote_code = True)
    #     model = AutoModel.from_pretrained(opt.llm_path, trust_remote_code = True).half().cuda() # device_map = "auto"
    #     tokenizer.bos_token_id, model.config.bos_token_id = tokenizer.eos_token_id, tokenizer.eos_token_id
    #     model.config.pad_token_id = tokenizer.pad_token_id
    elif opt.llm_path == "Salesforce/codegen-6B-mono" or opt.llm_path == "Salesforce/codegen-16B-mono": # for codegen
        tokenizer = AutoTokenizer.from_pretrained(opt.llm_path)
        model = AutoModelForCausalLM.from_pretrained(opt.llm_path, device_map = "auto").half()
        model.config.bos_token_id = tokenizer.bos_token_id
        tokenizer.pad_token_id = tokenizer.eos_token_id
        model.config.pad_token_id = tokenizer.eos_token_id
    elif opt.llm_path == "BAAI/AquilaCode-multi": 
        tokenizer = AutoTokenizer.from_pretrained(opt.llm_path, trust_remote_code = True)
        model = AutoModelForCausalLM.from_pretrained(opt.llm_path, device_map = "auto", trust_remote_code = True).half()
        tokenizer.pad_token_id = model.config.pad_token_id
        tokenizer.eos_token_id = model.config.eos_token_id
        tokenizer.bos_token_id = model.config.bos_token_id
    elif opt.llm_path == "tiiuae/falcon-7b" or opt.llm_path == "tiiuae/falcon-40b":
        from transformers import FalconForCausalLM
        tokenizer = AutoTokenizer.from_pretrained(opt.llm_path)
        if "-40b" in opt.llm_path:
            model = FalconForCausalLM.from_pretrained(opt.llm_path, device_map = "auto", torch_dtype = torch.bfloat16, load_in_8bit = True)
        else:
            model = FalconForCausalLM.from_pretrained(opt.llm_path, device_map = "auto", torch_dtype = torch.bfloat16)
        tokenizer.pad_token_id = tokenizer.eos_token_id
        model.config.pad_token_id = tokenizer.eos_token_id
        tokenizer.bos_token_id = tokenizer.eos_token_id
    elif "codellama" in opt.llm_path:
        tokenizer = AutoTokenizer.from_pretrained(opt.llm_path)
        model = AutoModelForCausalLM.from_pretrained(opt.llm_path, device_map = "auto", torch_dtype = torch.float16, use_safetensors = False)
        tokenizer.pad_token_id = tokenizer.eos_token_id
        model.config.pad_token_id = tokenizer.eos_token_id
    elif "codes" in opt.llm_path: # for codes
        tokenizer = AutoTokenizer.from_pretrained(opt.llm_path)
        model = AutoModelForCausalLM.from_pretrained(opt.llm_path, device_map = "auto", torch_dtype = torch.float16)
